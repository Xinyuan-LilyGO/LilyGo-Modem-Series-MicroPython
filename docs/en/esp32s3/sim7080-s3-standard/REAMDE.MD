...
<h1 align = "center">🌟LilyGo T-SIM7080G-S3-MicroPython-Standard🌟</h1>
...
8. ESP32S3 upload the MicroPython firmware:
   ...
   ```
   python -m esptool --chip esp32s3 --port COMX --baud 460800 --before=default_reset --after=hard_reset write_flash -z 0x0 D:\LilyGO-T-A76XX-Micropython-main\firmware\LilyGO-T-SIM7080G-S3-STAN-Micropython-firmware.bin
   ```

   Note：

   1. COMX is the port number. Change it to the port number corresponding to your computer.
   2. D:\LilyGO-T-A76XX-Micropython-main\firmware\LilyGO-T-SIM7080G-S3-STAN-Micropython-firmware.bin is the firmware path. Change it to the corresponding storage path.

...
from HivemqRootCA import HivemqRootCA
import re
...
def mqtt_connect(client_index, server, port, client_id, username=None, password=None,keepalive_time=60):
    ...
    if utilities.CURRENT_PLATFORM == "LILYGO_T_SIM7000G":
        ...
        print(response)
        response = send_at_command(f'AT+SMCONF="USERNAME","{mqtt_broker_username}"') 
        print(response)
        response = send_at_command(f'AT+SMCONF="PASSWORD","{mqtt_broker_password}"') 
        print(response)
        response = send_at_command(f'AT+SMCONF=URL,"{mqtt_broker}",8883') 
        print(response)
        if 'OK' in response:
            return True
        else:
        ...
        while True:
            if username and password:
                response_broker = send_at_command(f"AT+CMQTTCONNECT={client_index},\"tcp://{server}:{port}\",{keepalive_time},1,{username},{password}", wait=3)
                print(response_broker)
            else:
                response_broker = send_at_command(f"AT+CMQTTCONNECT={client_index},\"tcp://{server}:{port}\",{keepalive_time},1", wait=3)
                print(response_broker)
...
def mqtt_connecting(client_index, server, port, client_id, ssl, sni, keepalive_time=60):
    global __ssl, __sni, cert_pem, client_cert_pem, client_key_pem
    if utilities.CURRENT_PLATFORM == "LILYGO_T_SIM7000G":
        response = send_at_command("AT+SMDISC",wait=5)
        print(response)
...
def mqtt_subscribe(client_index, mqtt_publish_topic, qos=0, dup=0):
    if utilities.CURRENT_PLATFORM == "LILYGO_T_SIM7000G":
        response = send_at_command(f'AT+SMSUB="{mqtt_subscribe_topic}",0') 
        print(response)
    else:
...
def mqtt_publish(client_index, topic, message):
    if utilities.CURRENT_PLATFORM == "LILYGO_T_SIM7000G":
        response = send_at_command(f'AT+SMPUB="{mqtt_publish_topic}",10,0,1')
        print(response)
...
def main():
    ...
    mqtt_set_certificate(HivemqRootCA)
    
    if mqtt_connecting(client_index, mqtt_broker, mqtt_port, mqtt_client_id, ssl, sni):

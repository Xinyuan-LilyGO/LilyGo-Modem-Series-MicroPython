...
def mqtt_connect(client_index, server, port, client_id, username=None, password=None,keepalive_time=60):
    ...
    if username and password:
        response_broker = send_at_command(f"AT+CMQTTCONNECT={client_index},\"tcp://{server}:{port}\",{keepalive_time},1,{username},{password}", wait=5)
            ...
            while True:
                ...
                if current_millis > check_connect_millis:
                    ...
                    if not mqtt_connected():  # If not connected, attempt to reconnect
                        mqtt_connecting(client_index, mqtt_broker, mqtt_port, mqtt_client_id, ssl, sni)
                    else:

...
def loopGPS():
    global lon2,lat2,year2,month2,day2,hour2,min2,sec2,speed2
#     print("Requesting current GPS/GNSS/GLONASS location")
    ...
    if "+CGNSSINFO: ,,,,,,,," not in response and "ERROR" not in response:
        ...
        if len(values) >= 1:  
            ...
            print("The location was obtained successfully, and GPS positioning was turned off.")
            response = send_at_command("AT+CGNSSTST=0")
            print(response)
            response = send_at_command("AT+CGNSSPORTSWITCH=1,0")
            print(response)
...
def main():
    ...
    print(response)
    response = send_at_command("AT+CGNSSPORTSWITCH=0,1",wait=3)
    print(response)
